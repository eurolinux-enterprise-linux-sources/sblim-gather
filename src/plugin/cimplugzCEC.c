/*
 * $Id: cimplugzCEC.c,v 1.2 2009/05/20 19:39:56 tyreld Exp $
 *
 ...
 *
 * Description:
 * CIM plug in for the IBM system z CEC metrics
 */
...
CMPIObjectPath* COP4VALID (CMPIBroker *broker, const char *id, 
			   const char *systemid)
{
  CMPIObjectPath *cop = CMNewObjectPath(broker,NULL,"IBMz_CEC",
					NULL);
  if (cop) {
    CMAddKey(cop,"Name",id,CMPI_chars);
    CMAddKey(cop,"CreationClassName","IBMz_CEC",CMPI_chars);
  }
  return cop;
}
